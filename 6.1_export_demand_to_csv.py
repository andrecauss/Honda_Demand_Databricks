...
# ==============================================================================
# NOTEBOOK: 6.1 - Exportação de Demandas para CSV
# ==============================================================================
#
# PROPÓSITO:
#   Exportar TODAS as tabelas do schema parts_hdbk_sandbox.pr_demand como
#   .csv individuais, e empacotar tudo num único .tar.gz para distribuição.
#   Predecessor do 6.2 (Excel) — sem organização por 'file'/'sheet': cada
#   tabela do schema vira exatamente um .csv.
#
# ARQUITETURA:
#   ┌─────────────────────────────────────────────────────────────────────┐
#   │ INPUT: parts_hdbk_sandbox.pr_demand (todas as tabelas do schema)    │
#   └────────────────────────┬────────────────────────────────────────────┘
#                            │
#                            v
#   ┌─────────────────────────────────────────────────────────────────────┐
#   │ TRANSFORMAÇÃO:                                                      │
#   │   • df.coalesce(1).write.csv(...) por tabela — Spark grava uma      │
#   │     PASTA {tabela}.csv/ com part-*.csv + _SUCCESS                   │
#   │   • Reorganização: extrai o part-*.csv da pasta para {tabela}.csv   │
#   │     na raiz do volume e remove a pasta                              │
#   │   • Empacotamento: tar -czf de todos os .csv da raiz                │
#   └────────────────────────┬────────────────────────────────────────────┘
#                            │
#                            v
#   ┌─────────────────────────────────────────────────────────────────────┐
#   │ OUTPUT: /Volumes/parts_hdbk_sandbox/pr_demand/                      │
#   │         demand_refined_exportfiles/*.csv + *.tar.gz                 │
#   └─────────────────────────────────────────────────────────────────────┘
#
# CONVENÇÕES:
#   • Um .csv por tabela — sem multi-aba, sem agrupamento por 'file'
#   • Nome do arquivo = nome exato da tabela de origem
#
# DEPENDÊNCIAS:
#   • tar (via subprocess, precisa estar disponível no cluster)
#
# EXECUÇÃO:
#   1. Run All. Sobrescreve (overwrite) os .csv existentes a cada execução
#   2. Não há limpeza do volume antes — .csv de tabelas removidas do schema
#      ficam órfãos
#
...
print("📊 Notebook 6.1 - Exportação de Demandas para CSV carregado.")
