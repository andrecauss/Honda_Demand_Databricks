...
#   organizando os dados em múltiplas abas por arquivo quando a tabela carrega
#   as colunas de controle 'file' e 'sheet'.
#
# ARQUITETURA:
#   ┌─────────────────────────────────────────────────────────────────────┐
...
#   │   • Mapeamento tabela → (file, sheet) em duas consultas apenas      │
#   │   • Iteração por ARQUIVO: todas as tabelas de um 'file' viram abas  │
#   │     do mesmo workbook (TTL primeiro, centros em seguida)            │
#   │   • Coleta limitada ao driver (teto de linhas/células do Excel)     │
#   │   • Sanitização de nomes de aba e neutralização de fórmulas         │
#   │   • Montagem do workbook completo em memória (openpyxl)             │
#   └────────────────────────┬────────────────────────────────────────────┘
#                            │
#                            v
#   ┌─────────────────────────────────────────────────────────────────────┐
...
#   │   Gravação via Files API (staging local → upload binário)           │
#   └─────────────────────────────────────────────────────────────────────┘
#
...
#
# CONVENÇÕES:
#   • O gerador (5.1) grava UMA TABELA POR ABA, todas com o mesmo valor de
#     'file'. Logo, um arquivo .xlsx é montado a partir de VÁRIAS tabelas —
#     exportar tabela a tabela faria cada uma sobrescrever a anterior.
#   • Tabelas simples (sem 'file'/'sheet'): um workbook de aba única
#   • Nomes de arquivos: padrão "{SEGMENTO} {ANO} {MÊS} {TIPO}.xlsx"
#   • Ordem das abas: TTL (total consolidado) primeiro, centros depois
#   • Limite de 31 caracteres para nomes de abas Excel
#
# DEPENDÊNCIAS:
#   • openpyxl: escrita de arquivos .xlsx
#   • pandas: manipulação de dados tabulares (via toPandas)
#   • databricks-sdk: upload binário para o volume via Files API
#
# EXECUÇÃO:
#   1. Execute todas as células sequencialmente (Run All)
#   2. A célula de %pip install reinicia o Python — por isso ela vem antes
#      de qualquer variável de estado do notebook
#   3. O volume de destino é limpo (sem backup) antes de cada exportação
...
print("📊 Notebook 6.2 v2 - Exportação de Demandas para Excel carregado.")
